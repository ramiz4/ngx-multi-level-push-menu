...
export class MultiLevelPushMenuComponent implements OnDestroy {
  ...
  constructor(private router: Router, private multiLevelPushMenuService: MultiLevelPushMenuService) {
    this.initializeSubscription = this.multiLevelPushMenuService.initialized().subscribe(options => {
      this.initMenu(options);
    });
    this.updateSubscription = this.multiLevelPushMenuService.updated().subscribe(items => {
  ...
  updateMenu(items: Array<MultiLevelPushMenuItem>): void {
    if (JSON.stringify(this.oldMenuItems.sort()) !== JSON.stringify(items.sort())) {
      // Add new items
      const $addTo = $(this.elMenu.nativeElement).multilevelpushmenu('activemenu').first();
