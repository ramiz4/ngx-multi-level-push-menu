...
export class BrowserCompatibilityService {
  ...
  isSafari(): boolean {
    ...
    return (
      this.userAgent.includes('safari') && !this.userAgent.includes('chrome')
    );
  }
  ...
  isIE(): boolean {
    if (!this.isBrowser) return false;
    return (
      this.userAgent.includes('trident') || this.userAgent.includes('msie')
    );
  }
  ...
  isChrome(): boolean {
    if (!this.isBrowser) return false;
    return (
      this.userAgent.includes('chrome') && !this.userAgent.includes('edge/')
    );
  }
  ...
  isIOS(): boolean {
    if (!this.isBrowser) return false;
    return (
      /iPad|iPhone|iPod/.test(this.userAgent) ||
